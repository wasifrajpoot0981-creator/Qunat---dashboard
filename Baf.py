...
st.title("MEXC Balanced Quantitative Scalping Dashboard")
...
if bids_df is not None and asks_df is not None:
    ...
    # Medium & Balanced Signal Execution
    st.subheader("🎯 Scalping Signal Execution (Balanced Mode)")
    if avg_imb > 0.25 or hawkes > (baseline * 1.15) or laplace_prob > 0.62:
        st.success("🟢 **LONG SETUP (Balanced):** Solid buying pressure and volume momentum detected.")
    elif avg_imb < -0.25 or laplace_prob < 0.38:
        st.error("🔴 **SHORT SETUP (Balanced):** Solid selling pressure and downward momentum detected.")
    else:
        st.warning(f"⚪ **NEUTRAL / BALANCED ZONE:** Market moving steadily. Trend: {lstm_dir}")
