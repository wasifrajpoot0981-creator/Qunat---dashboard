...
st.title("MEXC Multi-Coin 1M High-Wall Momentum Engine — Pure Speed Setup")

symbol = st.selectbox("Select Trading Pair", ["BTCUSDT", "ETHUSDT", "SOLUSDT", "XRPUSDT", "BNBUSDT", "ADAUSDT"], index=0)

# --- MASTER QUANT FORMULA (Display Only) ---
...
if bids_df is not None and asks_df is not None and df_1m is not None:
    ...
    st.caption(f"🟢 **Live Server Timestamp:** {current_live_time} | **Mode:** Pure Speed & Direct Trigger")
    ...
    c6.metric("Quant Score", f"${master_quant_score:,.4f} (Info)")
    
    st.subheader("📊 High-Volume Organic Zones")
    ...
    # Final conditions: Master Quant Score removed from trigger checks entirely
    ...
    if long_condition:
        st.success(f"🟢 **HIGH WALL LONG SIGNAL ({symbol}):** Pure Speed Trigger! Support Reaction Active!")
    elif short_condition:
        st.error(f"🔴 **HIGH WALL SHORT SIGNAL ({symbol}):** Pure Speed Trigger! Resistance Reaction Active!")
    else:
        st.warning(f"⚪ **Scanning 1M High Walls ({symbol}):** Direct listening mode active...")
else:
