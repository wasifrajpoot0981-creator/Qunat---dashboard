...
# --- PAGE CONFIGURATION ---
st.set_page_config(
    page_title="Ultimate Quant Whale Ecosystem & Live Liquidation Dashboard",
    page_icon="⚡",
    layout="wide"
)
...
# --- 1. MEXC LIVE LIQUIDATION & SPIKE DETECTOR (STREAMLIT FRIENDLY) ---
def get_mexc_live_liquidations():
    liquidations_data = []
    try:
        # Fetching 24hr ticker data from MEXC to calculate sudden volume and price liquidations/spikes
        ...
        if isinstance(res, list):
            for item in res:
                ...
                if sym.endswith('USDT'):
                    ...
                    # Detecting severe liquidations / aggressive momentum shifts (-4% or +4% with high volume)
        ...
        # Sort by highest volume/impact
        liquidations_data = sorted(liquidations_data, key=lambda x: x['Volume'], reverse=True)
        return liquidations_data[:5] # Top 5 recent liquidations
    except:
        return []
...
def get_cryptoquant_prediction_signal(coin_ticker, current_price):
    ...
    try:
        ...
        if latest_whale_ratio >= 0.82:
            whale_intent = "🔴 FIRMS WANT TO DUMP (Institutional Selling)"
            cq_direction = 'SHORT'
        elif latest_whale_ratio < 0.62:
            whale_intent = "🟢 FIRMS WANT TO PUMP & ACCUMULATE"
            cq_direction = 'LONG'
        else:
...
# --- 3. OPTIMIZED QUANT ENGINE: ENTITY CO-CLUSTERING & MICRO-PRICE ---
def calculate_ultimate_quant_metrics(trades_df, bids_df, asks_df, current_price):
    try:
        ...
        firm_cluster_status = "Strict Institutional Cluster Verified" if avg_gamma > 0.45 else "Retail / Mixed Flow"
...
# --- 4. COINGECKO TOP 50 COINS & ENHANCED AI MODEL ---
...
# --- 5. SYNCHRONIZED SCANNER WITH LIQUIDATION FILTER ---
def scan_synchronized_market(recent_liquidated_symbols):
    ...
    raw_candidates = []
    coin_map = get_top_50_coin_symbols()
    top_50_symbols = list(coin_map.values())[:30]
        ...
        try:
            ...
            if 'bids' in depth_res and 'asks' in depth_res and isinstance(trades_res, list) and len(trades_res) > 10:
                ...
                if bids and asks:
                    ...
                    large_bids = bid_df[(bid_df['Qty'] >= (bid_median * 3.5)) & (bid_df['Price'] >= current_price * 0.985)]
                    large_asks = ask_df[(ask_df['Qty'] >= (ask_median * 3.5)) & (ask_df['Price'] <= current_price * 1.015)]
                    ...
                    # Boost score if symbol was recently liquidated (Liquidation-first logic)
                    ...
                    if not large_bids.empty and p_micro >= current_price * 0.997:
                        ...
                        if dist_pct <= 0.9:
                            ...
                            raw_candidates.append({
                                'Symbol': symbol,
                                'Type': 'LONG',
                                'Title': '🟢 TOP 50 LIQUIDATION & PUMP INTENT (LONG)',
                    ...
                    if not large_asks.empty and p_micro <= current_price * 1.003:
                        ...
                        if dist_pct <= 0.9:
                            ...
                            raw_candidates.append({
                                'Symbol': symbol,
                                'Type': 'SHORT',
                                'Title': '🔴 TOP 50 LIQUIDATION & DUMP INTENT (SHORT)',
                                'Current Price': current_price,
                                'Micro Price': p_micro,
                                'Level Price': wall_price,
                                'Level Qty': wall_qty,
    ...
    for cand in raw_candidates:
        ...
        score = 80.0 + cand['Liq_Bonus']
        reasons = ["Strict Iceberg Wall"]
        if cand['Liq_Bonus'] > 0:
            reasons.append("MEXC Liquidations Triggered")
        if order_dir == ai_dir:
            score += 10.0
            reasons.append("AI Model Confirmed")
        if order_dir == cq_dir:
            score += 8.0
...
def get_live_institutional_flows():
    ...
    for idx, (coin_id, ticker_base) in enumerate(tokens, 1):
        ...
        ai_conf = min(70.0 + (abs(v_index) * 28.0), 96.5)
...
# ==========================================
st.title("⚡ Ultimate Quant Whale Ecosystem & Live Liquidation Tracker")
st.markdown("Model workflow: Pehle **MEXC Live Liquidations** pakri jati hain, phir us base par **Top 50 Quant Sniper Signal** high accuracy ke sath generate hota hai!")

st.sidebar.title("⚡ System Control")
st.sidebar.info("Status: **ONLINE 🟢** | Engine: **MEXC Liquidations + AI Quant**")

# --- SECTION 1: LIVE MEXC LIQUIDATIONS & SPIKES ---
st.header("🌊 Live MEXC Liquidations & Volume Spikes (First Step)")
with st.spinner("Fetching real-time liquidations from MEXC API..."):
    live_liquidations = get_mexc_live_liquidations()

if live_liquidations:
    st.success("Live liquidations successfully captured! Is data ko ab main prediction model mein feed kiya ja raha hai.")
    ...
    st.info("Filhal market mein koi extreme liquidation spike nahi hai, standard orderbook scan jari hai.")
    liquidated_symbols = []

st.markdown("---")

# --- SECTION 2: TOP 50 QUANT WHALE SNIPER MASTER SIGNAL ---
st.header("🚀 Top 50 Quant Whale Sniper Master Signal (Optimized & Liquidated First)")
...
with st.spinner("Analyzing liquidations, Co-clustering variance, and AI predictions for Dashboard 1..."):
    candidates = scan_synchronized_market(liquidated_symbols)
...
if st.session_state.current_signal:
    ...
    if sig['Type'] == 'LONG':
        st.success(
            ...
            f"* **Liquidation & Confluence Status:** `{sig['Confluence_Desc']}`\n"
        ...
        st.error(
            ...
            f"* **CryptoQuant Intent:** `{sig['CQ_Status']}`\n"
            f"* **Micro-Price ($P_{{micro}}$):** `${sig['Micro Price']:,.4f}`\n"
            f"* **Final Decision:** `🔴 SHORT KARNA HAI` | **Leverage:** `40x` | **Expected Profit:** `+{sig['Profit']}%`\n"
            f"* **Probability Score:** `{sig['Probability']}%`\n"
            f"* **Live Price:** `${sig['Current Price']:,.4f}` | **Target Support:** `${sig['Target Price']:,.4f}`"
        )
else:
    st.warning("⏳ High-Accuracy filter active: Liquidated assets aur strict institutional walls match hone par signal show hoga...")

st.markdown("---")

# --- SECTION 3: INSTITUTIONAL FLOWS & TIME HORIZON ---
st.header("🌊 Institutional Flows & Expected Trade Horizon ('Kab Tak')")
if st.button("📡 Run Instant Institutional Flow Scan"):
    with st.spinner("Analyzing institutional flows & duration across assets..."):
        ...
        if inst_flows:
            ...
            st.dataframe(pd.DataFrame(inst_flows), use_container_width=True)
        else:
            st.info("Filhal koi extreme whale variance nahi mili.")

if os.path.exists(CSV_FILE_NAME):
    st.markdown("### 📁 Saved CSV History Records (`whale_signals_history.csv`)")
    csv_df = pd.read_csv(CSV_FILE_NAME)
    st.dataframe(csv_df.tail(15), use_container_width=True)
