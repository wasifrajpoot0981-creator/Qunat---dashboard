...
st.title("MEXC Multi-Coin 1M Scalping Dashboard with Price & Time Triggers")

symbol = st.selectbox("Select Trading Pair", ["BTCUSDT", "ETHUSDT", "SOLUSDT", "XRPUSDT", "BNBUSDT", "ADAUSDT"], index=0)

# --- MASTER MOMENTUM TIMER CLASS FROM SCREENSHOTS ---
class MasterMomentumTimer:
    ...
    def update_cvd(self, current_cvd):
        """Live trade execution delta storage"""
    ...
    def calculate_countdown(self, raw_wall_volume, cancellation_ratio=0.3):
        """
        Main Engine: Filters spoof and calculates countdown.
        cancellation_ratio: 0.0 to 1.0 (e.g., 0.3 means 30% orders spoof chal rahe hain)
        """
        if len(self.cvd_history) < 2:
            return "⏳ System Syncing CVD History..."
        
        # 1. Filter out the Spoof Volume
        real_wall_volume = raw_wall_volume * (1.0 - cancellation_ratio)
        
        # 2. Extract past CVD to get the slope velocity
        ...
        # 3. Calculate CVD Velocity (vCVD) -> Delta volume per second
        cvd_velocity = (current_cvd - past_cvd) / self.time_window
        
        if cvd_velocity <= 0:
            return "⚖️ Passive State: Buy flow lagging. Momentum postponed."
            
        seconds_to_impact = real_wall_volume / cvd_velocity
        minutes_to_impact = round(seconds_to_impact / 60.0, 1)
        
        # 5. Adaptive Alert Matrix
        if seconds_to_impact <= 3.0:
            return f"🚀 SYSTEM CRITICAL: Impact expected in {round(seconds_to_impact, 1)} seconds!"
        elif seconds_to_impact <= 60.0:
            return f"⏰ High Alert: Breakout within {round(seconds_to_impact, 1)} seconds ({minutes_to_impact} minutes)!"
        else:
            return f"⏳ Steady Accumulation: Estimated breakout in {minutes_to_impact} minutes."
...
if bids_df is not None and asks_df is not None and df_1m is not None:
    ...
    # Update Momentum Timer with live cumulative volume/loaf pressure
    ...
    # Get raw wall volume for the active side
    raw_wall_vol = asks_df['Volume'].max() if loaf_val < 0 else bids_df['Volume'].max()
    countdown_status = st.session_state.momentum_timer.calculate_countdown(raw_wall_volume=raw_wall_vol, cancellation_ratio=0.30)
    ...
    # Display Master Momentum Timer Countdown status
    st.info(f"⚡ **Master Momentum Timer & Spoof Status:** {countdown_status}")
    
    # --- CUSTOM PRICE & TIME TRIGGER SETTINGS ---
    ...
    st.subheader("🎯 1M Scalping Execution Signals (Optimized with Price Target)")
    
    # Condition checking including exact user price threshold triggers
    ...
    if long_condition:
        st.success(f"🟢 **1M HIGH-CONFIDENCE LONG EXPLORED ({symbol}):** Price reached target Long Level (${target_long_price:,.4f}) + LOAF Flow Confirmed + Fast Momentum Active!")
    elif short_condition:
        st.error(f"🔴 **1M HIGH-CONFIDENCE SHORT EXPLORED ({symbol}):** Price reached target Short Level (${target_short_price:,.4f}) + LOAF Flow Confirmed + Fast Momentum Active!")
    else:
        st.warning(f"⚪ **1M WAITING ZONE ({symbol}):** Waiting for price to hit target levels & matching momentum timer criteria...")
