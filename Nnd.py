...
# 1. Live MEXC Order Book Fetching Function with Error Tracking
def fetch_mexc_live_orderbook(symbol="BTCUSDT", limit=10):
    ...
    try:
        ...
        if "bids" in data and "asks" in data:
            ...
            st.error(f"Orderbook API Response Error: {data}")
            return None, None
    except Exception as e:
        st.error(f"Orderbook Connection Exception: {e}")
        return None, None

# 2. Fetch 15M Candlestick Data with Error Tracking
def fetch_15m_market_structure(symbol="BTCUSDT"):
    ...
    try:
        ...
        if isinstance(data, list) and len(data) > 0:
            ...
            st.error(f"Klines API Response Error: {data}")
            return None
    except Exception as e:
        st.error(f"Klines Connection Exception: {e}")
        return None
...
if bids_df is not None and asks_df is not None and df_15m is not None:
    ...
    st.warning("⚠️ Waiting for API Connection... (Agar yeh masla bar bar aaye toh samajh jayen ke Streamlit cloud ki IP block ya network slow hai).")
