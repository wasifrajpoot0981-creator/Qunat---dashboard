...
st.title("MEXC Multi-Coin 1M Scalping Dashboard")
...
if bids_df is not None and asks_df is not None and df_1m is not None:
    ...
    st.subheader("🎯 1M Scalping Execution Signals (Optimized)")
    
    at_support = (current_close >= support_level * 0.9995) and (current_close <= support_level * 1.0005)
    at_resistance = (current_close <= resistance_level * 1.0005) and (current_close >= resistance_level * 0.9995)
    
    medium_intensity = (jump_intensity >= 0.5) and (jump_intensity <= 4.0)
    valid_real_book = (slippage_cost > 0.0000001)
    
    # GARCH and VPIN removed from final condition checks to ensure fast 1M triggers
    short_condition = (
        at_resistance and 
        (loaf_val < -2.0) and 
        (p_short > p_long) and 
    ...
    if long_condition:
        st.success(f"🟢 **1M HIGH-CONFIDENCE LONG EXECUTION ({symbol}):** Support Reached + LOAF Flow Confirmed + Fast Momentum Active!")
    elif short_condition:
        st.error(f"🔴 **1M HIGH-CONFIDENCE SHORT EXECUTION ({symbol}):** Resistance Reached + LOAF Flow Confirmed + Fast Momentum Active!")
    else:
        st.warning(f"⚪ **1M WAITING ZONE ({symbol}):** Monitoring live 1-minute order flow, LOAF, and momentum intensity...")
