...
# --- 1. DASHBOARD 1: LIVE LIQUIDATIONS & VOLUME SPIKES ---
def get_mexc_live_liquidations():
    ...
    try:
        ...
        if isinstance(res, list):
            for item in res:
                ...
                if sym.endswith('USDT'):
                    ...
                    if abs(price_change) >= 2.0 and volume > 400000:
        ...
        return liquidations_data[:8]
    except:
        return []

# --- 2. CRYPTOQUANT ON-CHAIN WHALE INTENT ENGINE (FIRMS) ---
...
# --- 3. QUANT ENGINE & CO-CLUSTERING ---
def calculate_quant_metrics(trades_df, bids_df, asks_df, current_price):
    try:
        if trades_df.empty or bids_df.empty or asks_df.empty:
            return 0.5, 0.5, 0.0, "Standard Flow"
        ...
        cluster_status = "Institutional Cluster Verified" if avg_gamma > 0.35 else "Retail Flow"
        return p_micro, avg_gamma, sum(v_vals), cluster_status
    except:
        return current_price, 0.0, 0.0, "Normal Flow"

# --- 4. COINGECKO TOP 50 & AI MODEL ---
...
# --- 5. DASHBOARD 2: QUADRANT CHART MAPPING (FIRMS) ---
        ...
        try:
            ...
            if 'bids' in depth_res and 'asks' in depth_res and isinstance(trades_res, list) and len(trades_res) > 5:
                ...
                if bids and asks:
                    ...
                    p_micro, gamma_val, total_vol, cluster_desc = calculate_quant_metrics(trades_df, bid_df, ask_df, current_price)
                    
                    x_coord = max(min(price_change_pct * 1.5, 9.0), -9.0)
                    y_coord = max(min((total_vol / 1000.0), 9.0), -9.0)
                    q_color = 'cyan' if price_change_pct > 0 else 'orange'
                    ...
                    raw_candidates.append({
                        ...
                        'Title': '🟢 FIRMS MASTER SIGNAL (LONG)' if price_change_pct >= 0 else '🔴 FIRMS MASTER SIGNAL (SHORT)',
...
st.markdown("Yahan **Dashboard 1 (Orderbook & Live Liquidations)** aur **Dashboard 2 (Firms & Quadrant Map)** dono alag aur proper tarike se show ho rahe hain!")

st.sidebar.title("⚡ System Control")
st.sidebar.info("Status: **ONLINE 🟢** | Dual Dashboards Active")

# --- DASHBOARD 1: ORDERBOOK & LIVE LIQUIDATIONS ---
st.header("📊 Dashboard 1: Orderbook & Live MEXC Liquidations")
with st.spinner("Fetching live liquidations & orderbook data..."):
    live_liquidations = get_mexc_live_liquidations()

if live_liquidations:
    st.success("Live liquidations successfully loaded in Dashboard 1!")
    ...
    st.info("Scanning live orderbooks and volume spikes...")
    liquidated_symbols = []

st.markdown("---")

# --- DASHBOARD 2: FIRMS, QUADRANT MAP & MASTER SIGNAL ---
st.header("🚀 Dashboard 2: Firms (CryptoQuant Intent) & Master Quadrant Map")
...
with st.spinner("Analyzing Dashboard 2 firms quadrant map and generating master signals..."):
...
if st.session_state.current_signal:
    ...
    st.success(
        ...
        f"* **Confluence Status:** `{sig['Confluence_Desc']}`\n"
        f"* **Co-Clustering Variance ($\gamma$):** `{sig['Gamma_Score']}` ({sig['Cluster_Desc']})\n"
        f"* **Firms / CryptoQuant Intent:** `{sig['CQ_Status']}`\n"
    ...
    st.warning("⏳ Dashboard 2 scanning high-accuracy institutional signals...")
