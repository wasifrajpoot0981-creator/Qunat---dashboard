...
st.title("MEXC Ultimate Master Quantitative Dashboard")
...
def calculate_vw_obi(bids_df, asks_df, max_levels=5):
    ...
    for i in range(1, num_levels + 1):
        w_i = 1.0 / i  # Weighting factor inversely proportional to level distance
...
# 3. Large Order Aggressor Flow (LOAF) Calculation
def calculate_loaf(bids_df, asks_df, theta=10.0):
...
# 5. LSTM-style Sequential Trend Prediction
...
if bids_df is not None and asks_df is not None:
    ...
    loaf_val = calculate_loaf(bids_df, asks_df, theta=8.0)
    ...
    st.subheader("🚀 Master Quantitative & Stochastic Metrics")
    c1, c2, c3, c4 = st.columns(4)
    c1.metric("VW-OBI", f"{vw_obi:.4f}")
    c2.metric("LOAF Value", f"{loaf_val:.2f}")
    c3.metric("Hawkes Intensity", f"{hawkes:.2f}")
    c4.metric("LSTM Trend Score", f"{lstm_score:.4f} ({lstm_dir})")
    
    st.subheader("📊 Probability & Reaction Levels")
    ...
    # Big Momentum Signal Execution
    st.subheader("🎯 Big Momentum Signal Execution")
    if (vw_obi > 0.65 or p_long > 0.70) and hawkes > (baseline_vol * 1.10) and loaf_val >= 0:
        st.success("🟢 **BIG MOMENTUM BREAKOUT (LONG):** High VW-OBI, strong Hawkes intensity surge, and positive aggressor flow verified!")
    elif (vw_obi < -0.65 or p_short > 0.70) and hawkes > (baseline_vol * 1.10) and loaf_val <= 0:
        st.error("🔴 **BIG MOMENTUM BREAKOUT (SHORT):** High sell imbalance, Hawkes intensity surge, and negative aggressor flow verified!")
    else:
        st.warning(f"⚪ **CONSOLIDATION ZONE:** Waiting for institutional momentum. Current Trend: {lstm_dir}")
