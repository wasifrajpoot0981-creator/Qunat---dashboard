...
st.title("MEXC 15M Active Reversal & Momentum Dashboard")
...
# 5. LSTM-style Sequential Trend Prediction (Active Buy/Sell Tuning)
def lstm_predict_signal(obi_history):
    ...
    # Lowered threshold so LSTM actively shifts to Buy/Sell sooner
    if score > 0.10:
        return score, "Active Buy"
    elif score < -0.10:
        return score, "Active Sell"
...
if bids_df is not None and asks_df is not None:
    ...
    st.subheader("🚀 Active Reversal & Quantitative Metrics")
    ...
    # Signal Execution including Active LSTM Trend
    st.subheader("🎯 Active Signal Execution")
    if (vw_obi > 0.45 or p_long > 0.60 or lstm_dir == "Active Buy") and loaf_val >= 0 and hawkes > (baseline_vol * 1.01):
        st.success("🟢 **ACTIVE BUY SETUP:** Order flow imbalance and LSTM trend confirm buying pressure!")
    elif (vw_obi < -0.45 or p_short > 0.60 or lstm_dir == "Active Sell") and loaf_val <= 0 and hawkes > (baseline_vol * 1.01):
        st.error("🔴 **ACTIVE SELL SETUP:** Order flow imbalance and LSTM trend confirm selling pressure!")
    else:
        st.warning(f"⚪ **CONSOLIDATION / WAITING:** Market testing boundaries. LSTM Status: {lstm_dir}")
