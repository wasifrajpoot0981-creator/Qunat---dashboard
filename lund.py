...
st.title("MEXC Multi-Coin 1M High-Wall Momentum Engine — Ultra-Aggressive Setup")
...
# --- P_REAL AUTHENTICITY FORMULA (Ultra-Aggressive) ---
def calculate_order_authenticity_probability(age_sec, distance_val, fill_ratio, beta_0=0.0, beta_1=0.01, beta_2=-0.1, beta_3=0.5):
...
def calculate_hawkes_process(current_pressure, history_list, mu=0.7, alpha=0.3, beta=0.3):
...
# --- LOAF (LARGE ORDER IMBALANCE) FORMULA (Zero Friction Theta) ---
def calculate_loaf(bids_df, asks_df, theta=0.1):
    if bids_df.empty or asks_df.empty:
        return 0.0
...
def extract_high_walls_support_resistance(bids_df, asks_df):
    ...
    if otr_bid > 80: bids_adj.loc[0, 'Volume'] = 0.0
    if otr_ask > 80: asks_adj.loc[0, 'Volume'] = 0.0
...
if bids_df is not None and asks_df is not None and df_1m is not None:
    ...
    loaf_val = calculate_loaf(bids_df, asks_df, theta=0.1)
    ...
    p_real_score = calculate_order_authenticity_probability(age_sec=1.0, distance_val=active_distance, fill_ratio=min(max(abs(loaf_val), 0.01), 1.0))

    current_live_time = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())
    st.caption(f"🟢 **Live Server Timestamp:** {current_live_time} | **Mode:** Ultra-Aggressive & Maximum Speed")
    ...
    st.subheader("📊 High-Volume Organic Zones (Ultra-Aggressive)")
    ...
    # Maximum wide distance range (1.5%) for absolute early capture
    at_support = (current_close <= target_long_price * 1.015) and (current_close >= target_long_price * 0.985)
    at_resistance = (current_close >= target_short_price * 0.985) and (current_close <= target_short_price * 1.015)
    
    # Ultra-loose conditions: P_real > 20% and OTR < 80
    long_condition = at_support and (loaf_val > 0.01) and (p_long > p_short) and (p_real_score > 0.20) and (otr_bid < 80)
    short_condition = at_resistance and (loaf_val < -0.01) and (p_short > p_long) and (p_real_score > 0.20) and (otr_ask < 80)
    
    if long_condition:
        st.success(f"🟢 **HIGH WALL LONG SIGNAL ({symbol}):** Ultra-Fast Move Captured! Support Reaction Active!")
    elif short_condition:
        st.error(f"🔴 **HIGH WALL SHORT SIGNAL ({symbol}):** Ultra-Fast Move Captured! Resistance Reaction Active!")
    else:
        st.warning(f"⚪ **Scanning 1M High Walls ({symbol}):** Aggressive listening mode active...")
else:
