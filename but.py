...
# --- LIVE AUTO-REFRESH (Har 1 second baad live data & timestamps update honge) ---
st_autorefresh(interval=1000, limit=None, key="live_momentum_refresh")

st.title("MEXC Multi-Coin 1M High-Wall Momentum Engine — Live Real-Time Setup")
...
if bids_df is not None and asks_df is not None and df_1m is not None:
    ...
    st.caption(f"🟢 **Live Server Timestamp:** {current_live_time} | **Auto-Refreshing every 1s**")
    ...
    st.success(f"⏱️ **Live Momentum Timer Status:** {countdown_status}")
    ...
    if long_condition:
        ...
        st.warning(f"⚪ **Scanning 1M High Walls ({symbol}):** Waiting for live high probability wall reaction...")
else:
