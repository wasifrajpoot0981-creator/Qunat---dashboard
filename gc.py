...
# --- PAGE CONFIGURATION ---
st.set_page_config(
    page_title="MEXC Order Book + AI Whale Master Sniper",
    page_icon="🎯",
    layout="wide"
)
...
def get_top_50_coin_symbols():
    ...
    try:
        ...
        # Return uppercase symbols matching MEXC format (e.g., BTC, ETH -> BTCUSDT)
...
# Run AI check for a specific symbol list
def run_ai_whale_classification(target_symbols):
    coin_map = get_top_50_coin_symbols()
    ai_predictions = {} # Format: {'BTCUSDT': 'LONG' or 'SHORT'}
...
def scan_synchronized_market():
    ...
    raw_candidates = []
    all_coins = get_all_usdt_coins()[:40] # Scan top 40 liquid coins for performance
            ...
            if 'bids' in depth_res and 'asks' in depth_res and isinstance(trades_res, list) and len(trades_res) > 8:
                ...
                if bids and asks:
                    ...
                    if not valid_bids.empty:
                        ...
                        close_bids = valid_bids[valid_bids['Dist_Pct'] <= 0.2]
                        if not close_bids.empty and recent_sell > (total_traded_vol * 0.6):
                            ...
                            target_price = float(valid_asks_target.iloc[0]['Price']) if not valid_asks_target.empty else current_price * 1.015
                            ...
                            raw_candidates.append({
                                ...
                                'Leverage': 40,
                                'LeverageTag': '🎯 SYNCHRONIZED ENTRY (40x)',
                    ...
                    if not valid_asks.empty:
                        ...
                        close_asks = valid_asks[valid_asks['Dist_Pct'] <= 0.2]
                        if not close_asks.empty and recent_buy > (total_traded_vol * 0.6):
                            ...
                            target_price = float(valid_bids_target.iloc[0]['Price']) if not valid_bids_target.empty else current_price * 0.985
    ...
    # Now Match with AI Whale Classifier for Confluence
    ...
    for cand in raw_candidates:
        ...
        if sym in ai_signals:
            ...
            # Confluence Check: Order Book direction must match AI Whale direction
            if cand['Type'] == ai_dir:
                ...
                # If AI disagrees, give it a lower probability fallback instead of filtering out completely
...
# --- STREAMLIT UI ---
st.title("🎯 MEXC Order Book + AI Whale Synced Master Engine")
st.markdown("Yeh system **Order Book Depth** aur **CoinGecko Random Forest AI** ko aapas mein match karke sirf wahi trades deta hai jahan dono ka **Confluence (Match)** ho!")
...
with ui_container:
    ...
    with st.spinner("Scanning Order Books & Matching with AI Whale Predictions..."):
    ...
    # Display Current Live Synced Signal
    if st.session_state.current_signal:
        sig = st.session_state.current_signal
        
        st.markdown("---")
        st.subheader(f"🚨 SYNCHRONIZED MASTER SIGNAL (Order Book + AI Match)")
        ...
        if sig['Type'] == 'LONG':
            st.success(
                ...
                f"* **⭐ Combined Accuracy Probability:** `{sig['Probability']}%`\n"
                f"* **Leverage:** `{sig['LeverageTag']}`\n"
                f"* **Live Price:** `${sig['Current Price']:,.4f}` | **Support Zone:** `${sig['Level Price']:,.4f}` (Distance: `{sig['Distance %']}%`)\n"
            ...
            st.error(
                ...
                f"* **⭐ Combined Accuracy Probability:** `{sig['Probability']}%`\n"
                f"* **Leverage:** `{sig['LeverageTag']}`\n"
                f"* **Live Price:** `${sig['Current Price']:,.4f}` | **Resistance Zone:** `${sig['Level Price']:,.4f}` (Distance: `{sig['Distance %']}%`)\n"
                f"* **📊 Tape Volume:** `${sig['Volume']:,.2f}`\n"
        ...
        st.warning("⏳ Market mein exact order book absorption aur AI whale match ki talash ki ja rahi hai...")

    # Queue List View
    if len(candidates) > 1:
        st.subheader("🔮 Next Synced Queue Signals")
        for idx, item in enumerate(candidates[1:4], 1):
            ...
            st.info(
                ...
                f"* 🎯 **Zone:** `${item['Level Price']:,.4f}` | **Target:** `${item['Target Price']:,.4f}` | **Profit:** `+{item['Profit']}%`"
            )
