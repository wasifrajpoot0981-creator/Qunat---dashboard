...
# Page Configuration
st.set_page_config(
    page_title="Top 100+ Crypto Signal Scanner (MEXC)",
    page_icon="🐋",
    layout="wide"
)

st.title("🐋 MEXC Top 100+ Crypto 1-Min Long/Short Signal Scanner")
st.markdown("""
Yeh app MEXC exchange se live top 100+ USDT pairs ka data fetch karti hai. 
Market momentum aur price change ko analyze kar ke batati hai ke kis coin mein **LONG** banta hai aur kis mein **SHORT**.
""")
...
def fetch_mexc_data():
    try:
        # MEXC Public 24hr Ticker API endpoint
        ...
        if response.status_code == 200:
            ...
            # Volume ke hisaab se sort karein
...
with st.spinner("Fetching live market data from MEXC..."):
    df = fetch_mexc_data()
...
if df is not None:
    ...
    st.error("MEXC API se connect nahi ho pa raha. Internet connection check karein.")
