...
st.title("MEXC LOB Master Quantitative & Stochastic Dashboard")
...
# 4. Distance-based Queue Depths (Q_i^A, Q_i^B)
def calculate_queue_depths(bids_df, asks_df, distance_index=2):
    try:
        q_bid = bids_df.loc[distance_index, 'Volume'] if len(bids_df) > distance_index else 0.0
        q_ask = asks_df.loc[distance_index, 'Volume'] if len(asks_df) > distance_index else 0.0
        return q_bid, q_ask
    except:
        return 0.0, 0.0
...
# 6. Mid-Price Transition Probability via Laplace Transform
...
# 7. LSTM-style Sequential Trend Prediction
...
if bids_df is not None and asks_df is not None:
    ...
    q_bid, q_ask = calculate_queue_depths(bids_df, asks_df, 2)
    ...
    st.subheader("🚀 Master Quantitative Metrics Dashboard")
    c1, c2, c3, c4 = st.columns(4)
    c1.metric("LOB Imbalance", f"{avg_imb:.4f}")
    c2.metric("Realized Vol ($RV_\\alpha$)", f"{rv:.5f}")
    c3.metric("Hawkes Intensity", f"{hawkes:.2f}")
    c4.metric("Laplace Up Prob", f"{laplace_prob*100:.1f}%")
    
    st.subheader("📊 Queue Depths & Sequential Trends")
    q1, q2, q3 = st.columns(3)
    q1.metric("Queue Ask ($Q_i^A$)", f"{q_ask:.2f}")
    q2.metric("Queue Bid ($Q_i^B$)", f"{q_bid:.2f}")
    q3.metric("LSTM Trend Score", f"{lstm_score:.4f} ({lstm_dir})")
    
    # Combined Signal Execution Logic
    st.subheader("🎯 Master Scalping Signal Execution")
    if laplace_prob > 0.65 and avg_imb > 0.3 and lstm_dir == "Bullish":
        st.success("🟢 **STRONG LONG SETUP:** All stochastic, Hawkes, Laplace, and LSTM indicators align bullishly!")
    elif laplace_prob < 0.35 and avg_imb < -0.3 and lstm_dir == "Bearish":
        st.error("🔴 **STRONG SHORT SETUP:** All stochastic, Hawkes, Laplace, and LSTM indicators align bearishly!")
    else:
        st.warning(f"⚪ **NEUTRAL ZONE:** Current Trend: {lstm_dir}. Awaiting full model convergence.")
