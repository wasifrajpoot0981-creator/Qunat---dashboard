...
st.title("MEXC Multi-Coin 1M High-Wall Flip & Iceberg Catcher Engine (Dynamic)")
...
if 'tracked_symbol' not in st.session_state or st.session_state.tracked_symbol != symbol:
    ...
    st.session_state.init_vis_ask = None
...
if bids_df is not None and asks_df is not None and df_1m is not None:
    ...
    # Initialize baseline visible volume for real-time delta tracking
    if st.session_state.init_vis_bid is None:
        st.session_state.init_vis_bid = Vb
    if st.session_state.init_vis_ask is None:
        st.session_state.init_vis_ask = Va
    ...
    # Dynamic Iceberg Calculation using real tracking
    v_exec_proxy = current_1m_vol * 500
    v_canc_proxy = abs(st.session_state.init_vis_bid - Vb) * 0.5
    
    iceberg_phi = calculate_iceberg_intensity(
        v_exec_proxy, 
        st.session_state.init_vis_bid, 
        Vb, 
        v_canc_proxy
    )
    ...
    st.caption(f"🟢 **Live Server Timestamp:** {current_live_time} | **Engine:** Dynamic Tracking Mode")
