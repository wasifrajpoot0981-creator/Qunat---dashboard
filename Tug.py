...
st.title("MEXC 15M Balanced Quantitative Dashboard")
...
# 5. LSTM-style Sequential Trend Prediction (Medium Tuning at 0.15)
def lstm_predict_signal(obi_history):
    ...
    if score > 0.15:
        return score, "Medium Buy"
    elif score < -0.15:
        return score, "Medium Sell"
...
if bids_df is not None and asks_df is not None:
    ...
    st.subheader("🚀 Balanced Quantitative Metrics Dashboard")
    ...
    # Balanced Signal Execution
    st.subheader("🎯 Balanced Signal Execution")
    if (vw_obi > 0.50 or p_long > 0.62 or lstm_dir == "Medium Buy") and loaf_val >= 0 and hawkes > (baseline_vol * 1.02):
        st.success("🟢 **BALANCED LONG SETUP:** Order flow and medium LSTM trend confirming upward move.")
    elif (vw_obi < -0.50 or p_short > 0.62 or lstm_dir == "Medium Sell") and loaf_val <= 0 and hawkes > (baseline_vol * 1.02):
        st.error("🔴 **BALANCED SHORT SETUP:** Order flow and medium LSTM trend confirming downward move.")
    else:
        st.warning(f"⚪ **CONSOLIDATION ZONE:** Monitoring order book depth. LSTM Status: {lstm_dir}")
