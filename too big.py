...
def calculate_otr(v_placed, v_canceled, v_executed, eps=1e-9):
    return (v_placed + v_canceled) / (v_executed + eps)[cite: 14]
...
def calculate_loaf(bids_df, asks_df, theta=2.0):
    # Higher theta focuses purely on high volume walls
...
def extract_high_walls_support_resistance(bids_df, asks_df):
    ...
    # Top high wall evaluation with OTR check
    v_placed_b = bids_df['Volume'].iloc[0]
    otr_bid = calculate_otr(v_placed_b, v_placed_b * 0.2, max(bids_df['Volume'].mean() * 0.2, 0.001))[cite: 14]

    v_placed_a = asks_df['Volume'].iloc[0]
    otr_ask = calculate_otr(v_placed_a, v_placed_a * 0.2, max(asks_df['Volume'].mean() * 0.2, 0.001))[cite: 14]

    bids_adj, asks_adj = bids_df.copy(), asks_df.copy()
    if otr_bid > 40: bids_adj.loc[0, 'Volume'] = 0.0  # Zero out spoofed high walls[cite: 14]
    if otr_ask > 40: asks_adj.loc[0, 'Volume'] = 0.0  # Zero out spoofed high walls[cite: 14]

    # Pick highest volume true levels for maximum momentum response
...
def calculate_slippage_cost(bids_df, asks_df):
    if bids_df.empty or asks_df.empty: return 0.0
    return abs(asks_df.iloc[0]['Price'] / bids_df.iloc[0]['Price'] - 1)
...
if bids_df is not None and asks_df is not None and df_1m is not None:
    ...
    loaf_val = calculate_loaf(bids_df, asks_df, theta=2.0)
    ...
    lambda_intensity = calculate_hawkes_process(current_pressure, st.session_state.pressure_history)[cite: 15]
    ...
    c2.metric("OTR Bid/Ask", f"{otr_bid:.1f} / {otr_ask:.1f}")[cite: 14]
    c3.metric("Hawkes $\\lambda(t)$", f"{lambda_intensity:.2f}")[cite: 15]
    ...
    st.subheader("📊 High-Volume Organic Zones (Spoofing Filtered)")
    ...
    long_condition = at_support and (loaf_val > 1.5) and (p_long > p_short) and (p_real_score > 0.85) and (otr_bid < 40)[cite: 14]
    short_condition = at_resistance and (loaf_val < -1.5) and (p_short > p_long) and (p_real_score > 0.85) and (otr_ask < 40)[cite: 14]
    
    if long_condition:
        st.success(f"🟢 **HIGH WALL LONG SIGNAL ({symbol}):** Massive Volume Support + P_real > 85% Confirmed!")
    elif short_condition:
        st.error(f"🔴 **HIGH WALL SHORT SIGNAL ({symbol}):** Massive Volume Resistance + P_real > 85% Confirmed!")
    else:
